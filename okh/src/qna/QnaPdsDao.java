...
public class QnaPdsDao implements QnaPdsDaoImpl {
	...
	public boolean writePds(QnaPdsDto pds) {
		String sql = " INSERT INTO QNA_PDS( "
				+ " SEQ, ID, FILENAME,PARENT, REGDATE) "
				+ " VALUES(SEQ_QNA_PDS.NEXTVAL, "
				+ " ?, ?,?, SYSDATE) ";
	...
	public List<QnaPdsDto> getpdsList(int parent) {
		...
		PreparedStatement psmt = null;
		ResultSet rs = null;
		List<QnaPdsDto> list=new ArrayList<QnaPdsDto>();
		...
		try {
			...
			while(rs.next()){
				...
				QnaPdsDto dto = new QnaPdsDto(rs.getInt(1),
						rs.getString(2),
						rs.getString(3),
						rs.getInt(4),
						rs.getString(5)
	...
	@Override
	public QnaPdsDto getPdsBbs(int parent) {
		String sql = " SELECT SEQ, ID, FILENAME,PARENT, REGDATE "
				+ " FROM QNA_PDS "
				+ " WHERE PARENT=? ";
		...
		try {
			...
			if(rs.next()) {
				dto = new QnaPdsDto(rs.getInt(1),		//seq
						rs.getString(2),	//id
						rs.getString(3),	
						rs.getInt(4),
