...
import totalbbs.TotalController.CompareSeqDesc;
import user.UserDao;
...
public class TotalController extends HttpServlet {
	...
	public void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("totalbbs")) {
			...
			List<ColumnBbsDto> columnlist=tservice.allcolBbsList();
			...
			for (int i = 0; i < columnlist.size(); i++) {
				int wdate=0;
				ColumnBbsDto comdto=columnlist.get(i);
				String wdatee=comdto.getWdate();
				String mon=wdatee.substring(5, 7);
				String day=wdatee.substring(8, 10);
				String hou=wdatee.substring(11, 13);
				...
				String todate=mon+day+hou+min+sec;
				wdate=Integer.parseInt(todate);
				totallist.add(new totalbbsdto(comdto.getSeq(), "컬럼", comdto.getId(), 
						comdto.getTitle(),comdto.getContent(), comdto.getReadcount(), 0, 
						0, wdate));
			}
		...
		}else if(command.equals("othergivewritelist")) {
			...
			for (int i = 0; i < techlist.size(); i++) {
				...
				String todate=mon+day+hou+min+sec;
				wdate=Integer.parseInt(todate);
				insertid="-"+techdto.getId()+"-";
				totallist.add(new totalbbsdto(techdto.getSeq(), "기술게시판", insertid, 
						techdto.getTitle(),techdto.getContent(), techdto.getReadcount(), techdto.getLikecount(), 
						techdto.getCommentcount(), wdate));
			...
			for (int i = 0; i < lifelist.size(); i++) {
				...
				String todate=mon+day+hou+min+sec;
				wdate=Integer.parseInt(todate);
				insertid="-"+lifedto.getId()+"-";
				totallist.add(new totalbbsdto(lifedto.getSeq(), "사는얘기", insertid, 
						lifedto.getTitle(),lifedto.getContent(), lifedto.getReadcount(), lifedto.getUp(), 
						lifedto.getCountreply(), wdate));
			}
			for (int i = 0; i < bbs5list.size(); i++) {
				...
				String todate=mon+day+hou+min+sec;
				wdate=Integer.parseInt(todate);
				insertid="-"+bbs5dto.getId()+"-";
				totallist.add(new totalbbsdto(bbs5dto.getSeq(), "HW게시판", insertid,
						bbs5dto.getTitle(),bbs5dto.getContent(), bbs5dto.getReadcount(), bbs5dto.getLikecount(), 
						bbs5dto.getCommentcount(), wdate));
			...
			for (int i = 0; i < comlist.size(); i++) {
				...
				String todate=mon+day+hou+min+sec;
				wdate=Integer.parseInt(todate);
				insertid="-"+comdto.getId()+"-";
				totallist.add(new totalbbsdto(comdto.getSeq(), "커뮤니티게시판", insertid, 
						comdto.getTitle(),comdto.getContent(), comdto.getReadcount(), comdto.getJoinercount(), 
						comdto.getCommentcount(), wdate));
			}
			for (int i = 0; i < columnlist.size(); i++) {
				...
				wdate=Integer.parseInt(todate);
				insertid="-"+comdto.getId()+"-";
				totallist.add(new totalbbsdto(comdto.getSeq(), "컬럼", insertid, 
						comdto.getTitle(),comdto.getContent(), comdto.getReadcount(), 0, 
						0, wdate));
			}
			...
			sort=1;
			Collections.sort(totallist,new CompareSeqDesc());
			request.setAttribute("infoid", memid);
			request.setAttribute("writeserchlist", serchlist);
			dispatch("otherpage.jsp", request, response);
		}else if(command.equals("othergivelikelist")) {
			String memid=request.getParameter("memid");
			List<totalbbsdto> totallist=new ArrayList<>();
			List<TechbbsDto> techlist=tservice.getliketechBbsList(memid);
			List<LifeBbsDto> lifelist=tservice.getlikealllifeBbsList(memid);
			List<newbbs5HWCodingVO> bbs5list=tservice.getlikeallbbs5BbsList(memid);
			...
			Collections.sort(totallist,new CompareSeqDesc());
			request.setAttribute("infoid", memid);
			request.setAttribute("likelist", totallist);
			dispatch("otherpage.jsp", request, response);
		}else if(command.equals("givewritelist")) {
			String memid=request.getParameter("memid");
			System.out.println("들어왔는가");
			List<totalbbsdto> totallist=new ArrayList<>();
			List<TechbbsDto> techlist=tservice.alltechBbsList();
		...
		}else if(command.equals("givelikelist")) {
			String memid=request.getParameter("memid");
			List<totalbbsdto> totallist=new ArrayList<>();
			List<TechbbsDto> techlist=tservice.getliketechBbsList(memid);
			List<LifeBbsDto> lifelist=tservice.getlikealllifeBbsList(memid);
